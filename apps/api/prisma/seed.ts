...
async function main() {
  ...
  // Seed blog posts
  const blogAuthor = await prisma.user.findUnique({ where: { email: 'admin@earbudhub.com' } });
  if (blogAuthor) {
    const blogPosts = [
      {
        ...
        content: `The latest reports from industry insiders suggest that the AirPods Pro 3 will feature a revamped H3 chip, dedicated health sensors for heart rate monitoring, and a new "hearing aid mode" that leverages advanced on-device processing.
...
Additionally, we expect a 20% improvement in active noise cancellation (ANC) and better battery efficiency, potentially pushing playback time past 7 hours on a single charge.`,
...
In terms of sound, the Buds 3 Pro feature a sophisticated 2-way speaker system with a high-fidelity tweeter and a planar woofer, delivering crisp highs and deep, controlled bass.`,
...
The "Immersive Audio" mode, which uses head-tracking to simulate a spatial soundstage, remains a highlight of the experience.
...
Battery life remains its Achilles' heel, however. With Immersive Audio turned on, you can only expect about 4 hours of juice.`,
      ...
      },
      {
        ...
        content: `Huawei has quietly rolled out a service called "Loss Care" specifically for its FreeBuds product line. The concept is straightforward: pay a small fee when purchasing your FreeBuds, and if you lose a single earbud within the coverage period, Huawei will replace it at a significantly reduced cost.

**How it works:** After purchasing the Loss Care add-on, users register their FreeBuds through the Huawei Support app. If a component is lost, they can file a claim and receive a replacement unit.

**How it compares to PairAgain:** While Huawei's Loss Care is a manufacturer-backed insurance model, PairAgain's marketplace offers more flexibility across all brands.`,
