...
import { PrismaClient, ListingType, Condition } from '@prisma/client';
...
async function seedTestListings() {
  try {
    ...
    const listingStats = await Promise.all([
      prisma.listing.count({ where: { type: ListingType.EARBUD_PAIR } }),
      prisma.listing.count({ where: { type: ListingType.EARBUD_LEFT } }),
